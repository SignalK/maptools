...
public final class CoordinateConverter {
	...
	public double lon2x(double lon, double lat) {
		...
		double x = 0.0;
		double denom = 1.0 + sin(lat0Rad) * sin(latRad) + cos(lat0Rad) * cos(latRad) * cos(lonRad - Math.abs(lon0Rad));
		if (denom != 0.0) {
			x = ((2.0 * RADIUS) / denom) * cos(latRad) * sin(lonRad - Math.abs(lon0Rad));
		}
		return x;
	}
	...
	public double lat2y(double lon, double lat) {
		...
		double denom = 1.0 + sin(lat0Rad) * sin(latRad) + cos(lat0Rad) * cos(latRad) * cos(lonRad - Math.abs(lon0Rad));
		if (denom != 0.0) {
			y = ((2.0 * RADIUS) / denom) * (cos(lat0Rad) * sin(latRad) - sin(lat0Rad) * cos(latRad) * cos(lonRad - Math.abs(lon0Rad)));
		}
		return y;
	}
