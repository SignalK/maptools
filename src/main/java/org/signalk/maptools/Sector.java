...
public class Sector {
	...
	public static int getTileY(final double lat, final int zoom) {
		int ytile = (int)StrictMath.floor( (1 - StrictMath.log(StrictMath.tan(StrictMath.toRadians(lat)) + 1 / StrictMath.cos(StrictMath.toRadians(lat))) / StrictMath.PI) / 2 * (1<<zoom) ) ;
		// invert
